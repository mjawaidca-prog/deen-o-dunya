...
(function () {
  ...
  /* Some native plugin versions/methods return undefined instead of a Promise.
     Calling .catch() on that throws an uncaught TypeError with no React error
     boundary to stop it, which blanks the whole app. Guard every plugin call. */
  ...
  function setMetadata(meta) {
    ...
    if (plugin && plugin.setMetadata) {
      safeCall(function () { return plugin.setMetadata(payload); });
  ...
  function setPlaybackState(state) {
    ...
    if (plugin && plugin.setPlaybackState) {
      safeCall(function () { return plugin.setPlaybackState({ playbackState: state }); });
    } else if (haveWebMediaSession()) {
  ...
  function setHandlers(handlers) {
    ...
    if (plugin && plugin.setActionHandler) {
      actions.forEach(function (a) {
        safeCall(function () { return plugin.setActionHandler({ action: a[0] }, a[1]); });
      });
    } else if (haveWebMediaSession()) {
      actions.forEach(function (a) {
