...
(function () {
  ...
  // Android WebView has a known quirk where a detached `new Audio()` (never
  // inserted into the document) can decode and report "playing" while
  // producing no audible output on some devices. Attaching it to the DOM
  // (hidden) makes playback reliably audible.
  function attachToDom() {
    if (!document.body) { document.addEventListener("DOMContentLoaded", attachToDom, { once: true }); return; }
    [audio, preloader].forEach(function (el) {
